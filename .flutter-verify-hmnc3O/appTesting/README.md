...
3. Live Translator shows voice-to-text captions alongside the camera, reusable normalized landmark overlay, sign captions, tracking confidence, and view modes (Raw / Mesh / Clean).
4. Unknown signs have a separate “Sign not recognised” state that opens a five-valid-sample personal vocabulary flow.
5. Settings exposes real camera and speech-recognition controls, privacy switches, and camera recalibration. There is no obsolete conversation screen or saved-transcripts settings panel.
...
The generated platform files include the permission descriptions and declarations required by the camera and speech packages. iOS has `NSCameraUsageDescription`, `NSMicrophoneUsageDescription`, and `NSSpeechRecognitionUsageDescription`. Android has camera, record-audio, and internet permissions plus the speech recognition service query required on modern Android versions.

## Voice captions

Voice captions are available before camera calibration. On the first screen,
select **Use voice captions without camera**, then press **Start listening**,
speak one short message, and press **Stop listening** if the device does not
finish the utterance automatically. Interim words are cyan; a final result is
shown in white and can be selected or cleared. The recognizer defaults to the
device language, and another installed speech locale can be selected after
initialization.

`SpeechToTextService` owns the recognition lifecycle and exposes explicit
initializing, listening, ready, unavailable, and error states. A
`SpeechRecognizerAdapter` keeps platform channels out of service tests. The
service serializes start/stop operations, accepts final results after Stop,
ignores stale results after Cancel or Dispose, normalizes whitespace, and
deduplicates repeated final callbacks.

This feature is speech transcription: it turns spoken words into text for a
deaf or hard-of-hearing person to read. It does not translate between spoken
languages, and it does not yet generate sign-language animation. Recognition
uses the speech service configured by the operating system or browser and may
use an online provider. SignBridge does not send microphone audio through its
sign-language HTTP or WebSocket contracts. The current interaction is
deliberately utterance-based because mobile speech recognizers are not a
reliable always-on streaming API.
