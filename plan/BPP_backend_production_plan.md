...
| :---- | :---- |
| **Code** | `BPP` |
| **Status** | Ready to execute; all four production phases pending |
...
and non-secret decisions in the release ticket; place secret values only in the local shell or
Railway's sealed-variable UI.
...
Do not commit filled-in secret values, a copied `.env`, AWS credential files, certificates, private
keys, bearer tokens, or live lattice content.
...
5. Choose the credential path:
   ...
   - **Short hosted demo:** the temporary access-key ID, secret, and session token may be entered as
     sealed Railway variables, but the service stops working when they expire and must be refreshed.
...
| Client value | Source |
| :----------- | :----- |
| HTTPS base URL | Railway/custom production domain |
| WSS base URL | Same host with `wss` scheme |
| Language | Exact deployed `SIMPLYNEXT_RECOGNITION_LANGUAGE` |
